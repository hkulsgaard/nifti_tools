...
def askFiles(initialdir, title=None):

    filetypes = (('Nifti Image', ['*.nii','*.nii.gz']), ('Compressed Nifti Image', '*.nii.gz'),('All files', '*.*'))
    
    fnames = fd.askopenfilenames(filetypes=filetypes, initialdir = initialdir, title=title)
...
def list2str(my_list, separator='_'):
    
    list_string = separator.join(str(item) for item in my_list)
    return list_string
